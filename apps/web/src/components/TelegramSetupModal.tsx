...
import { Send, CheckCircle2, AlertCircle, Lock, ShieldCheck, X, RefreshCw, Key, Globe, ExternalLink } from 'lucide-react';
...
export const TelegramSetupModal: React.FC<TelegramSetupModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSendCode = async (e: React.FormEvent) => {
    ...
    if (!fullPhone || fullPhone.length < 8) {
      setError('Please enter a valid local phone number (e.g. 7766986183)');
      return;
    }
    ...
    if (!status?.is_configured && (!cleanApiId || !cleanApiHash)) {
      setError('Please enter your Telegram API ID (e.g. 37579496) and API Hash from my.telegram.org');
      return;
    }
    ...
    try {
      const resp = await apiClient.post('/telegram/send-code', {
        phone: fullPhone,
        apiId: cleanApiId || undefined,
        apiHash: cleanApiHash || undefined,
      });
  ...
  const handleVerifyCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (resp.data?.success) {
        setSuccessMsg(`Successfully authenticated as ${resp.data?.user?.first_name || 'Telegram User'}! Account credentials & session persisted.`);
        await fetchStatus();
          ...
          {loading && status === null ? (
            ...
            <div className="space-y-4">
              <div className="p-4 rounded-xl border border-emerald-500/30 bg-emerald-500/10 flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <div className="text-sm font-semibold text-emerald-300">
                    Telegram Authorized & Persisted
                  </div>
                  <div className="text-xs text-emerald-400/90 font-mono mt-0.5 truncate">
                    User: {status.user?.first_name} {status.user?.username ? `(@${status.user.username})` : ''} {status.user?.phone ? `(${status.user.phone})` : ''}
                  </div>
                </div>
              </div>
              ...
              <div className="text-xs text-[var(--color-text-muted)] leading-relaxed space-y-2">
                <p>
                  Your Telegram Telethon session string and credentials are saved locally in persistent storage.
                </p>
                <div className="p-2.5 rounded-lg bg-[var(--color-bg-overlay)] border border-[var(--color-border-subtle)] text-[11px] font-mono flex items-center justify-between">
                  <span>Configured API ID:</span>
                  <span className="text-[var(--color-brand-400)] font-semibold">{status.api_id || 'Active in backend'}</span>
                </div>
              </div>
            ...
            <form onSubmit={handleSendCode} className="space-y-4">
              <div className="p-3 rounded-lg bg-blue-500/10 border border-blue-500/20 text-xs text-blue-300 flex items-start gap-2">
                ...
                <div>
                  Enter your Telegram phone number. The Telegram scraper will send an OTP code directly to your Telegram mobile/desktop app.
                </div>
              </div>
              ...
              <div className="space-y-2 pt-1 border-t border-[var(--color-border-subtle)]">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="text-[11px] text-[var(--color-text-muted)]">API ID *</label>
                    <Input
                      type="text"
                      placeholder="e.g. 37579496"
                      value={apiId}
                      onChange={(e) => setApiId(e.target.value)}
                      required={!status?.is_configured}
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[11px] text-[var(--color-text-muted)]">API Hash *</label>
                    <Input
                      type="text"
                      placeholder="e.g. 0123456789abcdef..."
                      value={apiHash}
                      onChange={(e) => setApiHash(e.target.value)}
                      required={!status?.is_configured && !status?.api_hash_set}
                    />
                  </div>
                </div>
              </div>
