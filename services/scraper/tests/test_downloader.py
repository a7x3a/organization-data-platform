...
def test_extract_filename_falls_back_to_unknown_for_empty_path():
    assert extract_filename("https://example.com/") == "unknown"


@respx.mock
async def test_downloads_and_hashes_content_correctly(temp_dir):
...
@respx.mock
async def test_rejects_file_exceeding_max_size(temp_dir):
    content = b"x" * 1000
...
@respx.mock
async def test_rejects_empty_response(temp_dir):
...
@respx.mock
async def test_rejects_non_200_status(temp_dir):
...
@respx.mock
async def test_two_different_urls_same_content_hash_identically(temp_dir):
    # This is the whole basis of exact deduplication (spec §11).
    content = b"identical bytes regardless of source URL"
    respx.get("https://example.com/a.pdf").mock(return_value=httpx.Response(200, content=content))
    respx.get("https://example.com/b.pdf").mock(return_value=httpx.Response(200, content=content))
